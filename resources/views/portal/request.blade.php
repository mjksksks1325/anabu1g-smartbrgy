...
<title>Request a Document | Barangay Anabu I-G</title>
...
<div class="container" id="main-content" tabindex="-1">
  <div class="portal-request-back"><a href="{{ route('home') }}">&larr; Back to home</a></div>
  <section class="portal-request-flow" id="request-flow" aria-label="Document request">
  <div class="portal-workflow-heading"><div class="portal-eyebrow">Online document request</div><h2>Start your request</h2><p>Review the terms, choose a document, and submit your details for barangay review.</p></div>
    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="cert-grid" id="cert-type-grid">
          ...
          <div class="cert-btn" role="button" tabindex="0" aria-pressed="false" onclick="selectDoc('{{ $service->portalCode() }}',this)"><div class="icon">&#128196;</div><div class="label">{{ $service->value }}</div><div class="fee">{{ $service->fee() === 0 ? 'Free' : 'PHP '.number_format($service->fee(), 2) }}</div><div class="days">Confirm processing time with the barangay</div></div>
          @endforeach
        </div>
  ...
  <div class="screen" id="screen-form">
    ...
    <div class="card">
      <div class="card-header">
        ...
        <p>Identity comes from your official resident record. Contact the barangay for corrections.</p>
      </div>
      <form class="card-body" id="resident-details-form" autocomplete="off" novalidate onsubmit="event.preventDefault();goToAttachment()">
        ...
        <div class="form-group">
          ...
          <input class="form-input" id="f-name" readonly aria-readonly="true" required maxlength="255" enterkeyhint="next" placeholder="Halimbawa: Juan dela Cruz" autocomplete="off"/>
        ...
        <div class="form-group">
          ...
          <input class="form-input" id="f-address" readonly aria-readonly="true" required maxlength="1000" autocomplete="off" enterkeyhint="next" placeholder="Purok, Sityo o Street — Barangay Anabu I-G"/>
        </div>
        ...
        <div class="form-group">
          ...
          <input class="form-input" id="f-email" readonly aria-readonly="true" maxlength="255" inputmode="email" autocapitalize="none" spellcheck="false" enterkeyhint="next" placeholder="example@gmail.com" type="email" autocomplete="off" required/>
...
<footer class="footer portal-footer"><div><strong>Barangay Anabu I-G</strong><span>City of Imus, Cavite &middot; Resident Services Portal</span></div><button class="btn btn-outline" type="button" onclick="showScreen('screen-status')">Check request status &rarr;</button></footer>

<script>
const API = '';
const DOC_TYPES = {{ Illuminate\Support\Js::from(collect($services)->mapWithKeys(fn ($service) => [$service->portalCode() => ['label' => $service->value, 'icon' => '', 'fee' => $service->fee() === 0 ? 'Free' : 'PHP '.number_format($service->fee(), 2), 'days' => 'Confirm processing time with the barangay']])) }};
