...
function openDocumentRequestModal(
    ...
    document.getElementById('dr-reference').value = referenceCode || '';
    document.getElementById('dr-name').value = fullName || '';
    document.getElementById('dr-document').value = documentType || '';
    document.getElementById('dr-dob').value = dateOfBirth || '';
    document.getElementById('dr-address').value = address || '';
    document.getElementById('dr-contact').value = contactNumber || '';
    document.getElementById('dr-purpose').value = purpose || '';
    document.getElementById('dr-business').value = businessName || '';
    document.getElementById('dr-status').value = status || 'pending';
    document.getElementById('dr-remarks').value = remarks || '';
    
    const issueButton = document.getElementById('dr-issue-certificate');

    if (issueButton) {
        if (status === 'approved' || status === 'ready_for_release') {
            issueButton.style.display = 'inline-flex';
        } else {
...
function updateRequestStage(id, status) {
...
function filterRequestBoard(value) {
    const search = (value || '').toLowerCase().trim();

    document.querySelectorAll('.request-card').forEach(card => {
        const searchableText =
            (card.dataset.requestSearch || '').toLowerCase();

        card.style.display =
            searchableText.includes(search) ? '' : 'none';
    });
}

function filterDocumentRequests(searchValue) {
    const search = searchValue.toLowerCase().trim();

    const table = document.querySelector(
        '#screen-certificates .tbl'
    );
...
function filterDocumentRequestStatus(statusValue) {
    const table = document.querySelector(
        '#screen-certificates .tbl'
    );
    ...
    rows.forEach(row => {
        ...
        if (
            statusValue === 'all' ||
            rowStatus === statusValue.toLowerCase()
        ) {
            row.style.display = '';
        } else {
            row.style.display = 'none';
...
function filterDocumentRequestsTable() {
    const searchInput = document.getElementById('document-request-search');
    const statusFilter = document.getElementById('document-request-status-filter');

    const search = (searchInput?.value || '').toLowerCase().trim();
    const selectedStatus = (statusFilter?.value || 'all').toLowerCase().trim();

    const table = document.querySelector('#screen-certificates .tbl');
    if (!table) return;
    ...
    rows.forEach(row => {
        ...
        const rowStatus = cells[3].textContent.toLowerCase().trim();
...
function openIssueCertificateFromRequest() {
    if (!currentDocumentRequest) return;

    console.log(currentDocumentRequest.documentType);

    document.getElementById('issue-document-request-id').value =
        currentDocumentRequest.id || '';

    const certificateType = document.getElementById('issue-certificate-type');
    ...
    const requestType = normalizeType(currentDocumentRequest.documentType);

    let matched = false;

    Array.from(certificateType.options).forEach(option => {
        const optionValue = normalizeType(option.value);
        const optionText = normalizeType(option.textContent);

        if (optionValue === requestType || optionText === requestType) {
            certificateType.value = option.value;
            matched = true;
        }
    });

    if (!matched) {
        console.log('No certificate type match for:', currentDocumentRequest.documentType);
    }
    ...
    const normalizedPriceType = normalizeType(
        currentDocumentRequest.documentType
    );

    const paymentInput = document.getElementById('issue-amount-paid');

    if (paymentInput) {
        paymentInput.value = priceMap[normalizedPriceType] ?? 0;
    }

    document.getElementById('issue-resident-name').value =
        currentDocumentRequest.fullName || '';

    document.getElementById('issue-purpose').value =
        currentDocumentRequest.purpose || '';

    closeModal('modal-document-request');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (params.get('screen') === 'certificates') {
        const certNav = document.querySelector(
            '[onclick*="showScreen(\'certificates\'"]'
        );

        showScreen('certificates', certNav);
    }
});